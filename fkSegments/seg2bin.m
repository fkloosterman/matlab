...
switch options.method
 case 'binsize'
  ...
  elseif ~isnumeric(options.binsize) || ~isscalar(options.binsize)
    ...
    nbins = ( diff(seg,1,2) - options.overlap.*options.binsize ) ./ ((1-options.overlap).*options.binsize);
    tmpidx = (ceil(nbins) - nbins) < options.tol;
    nbins(tmpidx) = ceil(nbins(tmpidx));
    nbins(~tmpidx) = floor(nbins(~tmpidx));
  end
