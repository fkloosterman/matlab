...
%    segments - limit the search for mountains to specified segments
%
...
if isempty(options.threshold)
  if ~isempty(options.segments)
      options.threshold = quantile( sig( inseg( options.segments, x ) ), 0.9 );
  else
...
if ~isempty(options.segments)
   segs = segs( inseg( options.segments, segs, 'partial' ), : );
end
